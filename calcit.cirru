      ...
      :defs $ {}
        'comp-helicoid $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defcomp comp-helicoid (states)
            let
              ...
              group ({})
                tube $ {} (:points-fn helicoid-fn) (:factor state) (:radius 0.2)
                  :tubular-segments 800
                  :radial-segments 12
                  :position $ [] 0 0 0
                  :material $ {} (:kind :mesh-standard) (:color 0xcccc77) (:opacity 1) (:transparent true)
                tube $ {} (:points-fn helicoid-fn-2) (:factor state) (:radius 0.2)
        ...
        'rotate-wave $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn rotate-wave (dx bend v)
            let
                q1 $ ffi-object $ new THREE/Quaternion (nth v 0) (nth v 1) (nth v 2) 0
                q2 $ ffi-object $ new THREE/Quaternion 0 0 0
              .!setFromAxisAngle q2 (new THREE/Vector3 1 0 0) (* bend &PI dx)
              ; js/console.log q2
              let
                  q3 $ ffi-object $ .!premultiply q1
                    ffi-object $ .!invert $ ffi-object (.!clone q2)
        ...
        'surface-fn $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn surface-fn (t d state)
            let
                ...
                angle $ &* t &PI
                rot-angle $ &* v $ squeezing-01 t
                out-r $ &* r $ js-number (js/Math.tan angle)
                distance $ &/ r $ js-number (js/Math.cos angle)
                y0 $ &- r distance
                angle2 $ &- (&* 0.5 &PI) angle
